...
export function Portfolio() {
  ...
  return <div className="site-shell">
    ...
    <main>
      ...
      <section id="testimonials" className="section container"><Reveal><SectionTitle eyebrow="08 / Testimonials" title="Words from people I’ve worked with." intro="This optional section is prepared for genuine recommendations. Placeholder content is never presented as a real endorsement." /></Reveal><div className="testimonial-scroll">{testimonials.map((t,i)=><article className="testimonial" key={i}><span>“</span><p>{t.testimonial}</p><div><i>{t.name.slice(0,2)}</i><p><b>{t.name}</b><small>{t.designation} · {t.company}</small></p></div></article>)}</div></section>
